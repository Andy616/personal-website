...
const Skill: FC<{ skill: SkillType }> = memo(function Skill({ skill }) {
  return (
    <>
      <Typography component="div">{skill.name}</Typography>
      <BorderLinearProgress variant="determinate" value={skill.level} />
    </>
  );
});

const SkillGroup: FC<{ skillGroup: SkillGroupType }> = memo(
...
const Skills: FC<{ skillGroups: SkillGroupType[] }> = memo(function Skills({
  skillGroups,
}) {
    ...
    <>
      {isMobile ? (
        <SkillsMobile
          ...
          coverflowEffect={coverflowMobile}
          slideWidth={slideWidthMobile}
        />
      ) : (
        <SkillsDesktop
          skillGroups={skillGroups}
          coverflowEffect={coverflowDesktop}
          slideWidth={slideWidthDesktop}
        />
      )}
    </>
  );
